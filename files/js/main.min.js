...
function loadData(data) {
	workContainer = "#content-container .work-list ul"
	for (i=0; i<data.length; i++) {
		$(workContainer).append(
			`<li class="list-item" data-id="${data[i].id}">
				<p class="descriptor w-animate">${data[i].date}</p>
				<div class="inline-wrapper">
					<div class="w-animate title-wrapper">
						<p class="item-title overlay">${data[i].title}</p>
...
function workClickListener() {
	...
	function openItem(id) {
		...
		$("#listing-container .item-date").text(DATA[id].date)
		$("#listing-container .item-title p").text(DATA[id].title)
		for (i=0; i<DATA[id].roles.length; i++) {
			$("#listing-container .role-container ul").append(`<li class="role">${DATA[id].roles[i]}</li>`)
		}
		for (i=0; i<DATA[id].links.length; i++) {
			$("#listing-container .links").append(`<a class="link-btn ${DATA[id].links[i].type}" href="${DATA[id].links[i].link}" target="_blank"><span>${DATA[id].links[i].text}</span></li>`)
		}
